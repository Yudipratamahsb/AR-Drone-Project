...
void wrapperCallback (_ftp_status status, void *arg, _ftp_t *ftp)
{
    ...
    if (nil != ardftp)
    {
        ...
        if (FTP_BUSY != locStat && FTP_PROGRESS != locStat) // Keep FTP busy if it was already busy before call, or if the current process is still in progress 
        {
            ardftp.isBusy = NO;
        }
...
{
    NSString *ip = [NSString stringWithCString:&ardrone_info.drone_address[0] encoding:NSUTF8StringEncoding];
    const int port = 5551;
