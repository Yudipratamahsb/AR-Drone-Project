...
#if [ ${PLATFORM_PREFERRED_ARCH:0:4} = "armv" ]; then
#make RELEASE_BUILD=$RELEASE_BUILD iphone
#export FFMPEG_LIBRARY_PATH="../../ARDroneLib/Soft/Build/targets_versions/ffmpeg_iphone_"$PLATFORM_BUILD_FORMAT"_iphoneos_DeveloperPlatforms"$PLATFORM_NAME_FORMAT".platformDeveloperusrbinarm-apple-darwin10-gcc-4.2.1_4.2.1"
#else
#make RELEASE_BUILD=$(RELEASE_BUILD)
#export FFMPEG_LIBRARY_PATH="../../ARDroneLib/Soft/Build/targets_versions/ffmpeg_host_"$PLATFORM_BUILD_FORMAT"_DeveloperPlatforms"$PLATFORM_NAME_FORMAT".platformDeveloperusrbinarm-apple-darwin10-gcc-4.2.1_4.2.1"
#fi
...
grep -r ^ARDRONE_CONFIG_KEY_ $KEY_FILE | grep K_WRITE | grep -e control -e network -e video -e leds -e detect -e gps -e userbox | tr ',' ' ' >> $TEMP_FILE
awk '
BEGIN {
print "// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!";
